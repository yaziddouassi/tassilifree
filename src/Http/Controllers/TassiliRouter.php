...
namespace Tassili\Free\Http\Controllers;
...
class TassiliRouter extends Controller
{
   ...
   public function update(Request $request)
    ...
    $record =  \Tassili\Free\Models\TassiliCrud::find($request->id);
...
if ($request->filled('panel')) {
     $panels = \Tassili\Free\Models\TassiliCrud::where('route', 'like','%'.$request->panel.'%')
             ->paginate(10) ;
}

if (!$request->filled('panel')) {
     $panels = \Tassili\Free\Models\TassiliCrud::paginate(10) ;
}
    ...
    {  
        \Tassili\Free\Models\TassiliCrud::where('id',$request->id)->update([
    ...
    {  
        \Tassili\Free\Models\TassiliCrud::destroy($request->id);
    }  
    ...
    public function creator(Request $request)
    {
        ...
        foreach ($request->selected as $key => $value) {
            ...
            $transformString = new \Tassili\Free\Utils\TransformString();
